...
# Script to run the pipeline for financial causal attributions
...
# Log file
LOG_FILE="${LOGS_DIR}/run_train_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "===== Starting Financial Causal Attributions Pipeline with Training ====="
echo "Date: $(date)"
